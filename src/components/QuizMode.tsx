...
import { CheckMark, ChalkStar } from './Doodles';
...
export function QuizMode() {
  ...
  if (completed) {
    return <div className="h-full p-8 flex justify-center items-center">
        <motion.div initial={{
        scale: 0.9,
        opacity: 0
      }} animate={{
        scale: 1,
        opacity: 1
      }} className="bg-white p-12 rounded-xl shadow-lg text-center max-w-md w-full relative overflow-hidden">
          <div className="absolute inset-0 bg-paper-pattern opacity-30 pointer-events-none"></div>
          <ChalkStar className="w-24 h-24 text-yellow-500 mx-auto mb-6" />
          <h2 className="font-hand text-4xl font-bold text-school-board mb-4">
            {t('quiz_complete')}
          </h2>
          <p className="font-serif text-xl text-stone-600 mb-8">
            {t('quiz_score_msg', { score, total: localQuestions.length })}
          </p>
          <button onClick={handleRestart} className="flex items-center justify-center gap-2 w-full px-6 py-3 bg-school-board text-white rounded-lg font-hand font-bold text-xl hover:bg-school-board/90 transition-colors">
            <RefreshCw size={20} />
            {t('try_again')}
          </button>
        </motion.div>
  ...
  return <div className="h-full p-8 flex justify-center items-start overflow-y-auto">
      <motion.div key={currentQIndex} className="bg-white max-w-2xl w-full shadow-lg p-8 md:p-12 relative min-h-[500px]" initial={{
      x: 20,
      opacity: 0
    }} animate={{
      x: 0,
      ...
      opacity: 0
        ...
        <div className="border-b-2 border-black pb-4 mb-8 flex justify-between items-end">
          <div>
            ...
            <p className="font-hand text-lg text-stone-500">
              {t('question_progress', { current: currentQIndex + 1, total: localQuestions.length })}
            </p>
          </div>
          <div className="border-2 border-school-board w-16 h-16 rounded-full flex items-center justify-center opacity-80 bg-stone-50">
            <span className="text-school-board font-hand text-xl font-bold">
        ...
        {/* Question */}
        <div className="mb-8">
          <p className="font-serif text-xl font-medium mb-6 leading-relaxed" dir="auto">
          ...
          <div className="space-y-4">
            {currentQuestion.options.map((opt: string, idx: number) => <button key={idx} onClick={() => !submitted && setSelected(idx)} disabled={submitted} className={`
                  w-full text-start flex items-center gap-4 p-4 rounded-lg transition-all border-2 group
