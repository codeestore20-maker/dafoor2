...
export function Flashcards() {
  ...
  return <div className="h-full flex flex-col items-center justify-center p-6 relative">
      {/* Top Controls */}
      <div className="absolute top-6 right-6 rtl:right-auto rtl:left-6 flex items-center gap-4">
      ...
      {/* Progress Bar */}
      <div className="absolute top-6 left-6 rtl:left-auto rtl:right-6 w-32 h-2 bg-stone-200 rounded-full overflow-hidden">
      ...
      <div className="w-full max-w-md perspective-1000">
        <div className="relative h-72 w-full cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
          ...
          damping: 20
            ...
            <div className="absolute inset-0 backface-hidden">
              <IndexCard className="h-full flex items-center justify-center text-center p-8">
                <h3 className="font-hand text-3xl text-school-board font-bold" dir="auto">
                  {currentCard.front}
                </h3>
            ...
            transform: 'rotateY(180deg)'
              ...
              <IndexCard className="h-full flex items-center justify-center text-center p-8 bg-yellow-50">
                <h3 className="font-hand text-3xl text-school-red font-bold" dir="auto">
                  {currentCard.back}
                </h3>
              </IndexCard>
            </div>
      ...
      <div className="mt-12 flex items-center gap-6">
        <button onClick={handleMistake} className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-stone-200 text-stone-600 rounded-full font-hand text-xl shadow-sm hover:bg-stone-50 hover:border-stone-300 transition-all">
          <RotateCcw size={18} className="flip-rtl" />
          {t('review_again')}
        </button>

        <button onClick={handleMastered} className="flex items-center gap-2 px-8 py-3 bg-school-board text-white rounded-full font-hand text-xl shadow-lg hover:bg-opacity-90 transition-all transform hover:scale-105">
          <Check size={20} />
          {t('mastered')}
        </button>
      </div>
